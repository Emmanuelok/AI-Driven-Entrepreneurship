...
// GET    → read the full venture (any member)
// PATCH  → update data (owner + editor only). Body: { data, ifVersionAt? }
// DELETE → remove the venture entirely (owner only)
...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await sb.from("cloud_ventures").select("id, owner_id, name, data, updated_at").eq("id", id).maybeSingle();
  if (error) return Response.json({ ok: false, error: error.message }, { status: 500 });
  if (!data) return Response.json({ ok: false, error: "not_found" }, { status: 404 });

  return Response.json({ ok: true, venture: data, myRole: me.role });
}
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const token = (req.headers.get("authorization") || "").replace(/^Bearer\s+/i, "");
  const me = await authVenture(token, id);
  const forbidden = requireRole(me, "editor");
  ...
  // Optional optimistic-concurrency check: bail if the server has been
  // updated since the client read.
  if (body.ifVersionAt) {
    const { data: current } = await sb.from("cloud_ventures").select("updated_at").eq("id", id).maybeSingle();
  ...
  const { data, error } = await sb.from("cloud_ventures").update(patch).eq("id", id).select("updated_at").maybeSingle();
  if (error) return Response.json({ ok: false, error: error.message }, { status: 500 });
...
export async function DELETE(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await sb.from("cloud_ventures").delete().eq("id", id);
  if (error) return Response.json({ ok: false, error: error.message }, { status: 500 });
