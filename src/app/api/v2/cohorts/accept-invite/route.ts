...
// Redeem an invite token → become a cohort member with the invited role.
...
export async function POST(req: Request) {
  ...
  const { data: invite } = await sb.from("cohort_invites").select("*").eq("token", inviteToken).maybeSingle();
  ...
  const { error: addErr } = await sb.from("cohort_members").upsert({
  ...
  await sb.from("cohort_invites").delete().eq("id", invite.id);
  return Response.json({ ok: true, cohortId: invite.cohort_id, role: invite.role });
}
